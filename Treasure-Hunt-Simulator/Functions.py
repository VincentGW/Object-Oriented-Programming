...
general_font = pg.font.Font('Assets/Font/PressStart2P-Regular.ttf', 14); instruction_font = pg.font.Font('Assets/Font/PressStart2P-Regular.ttf', 14); small_font = pg.font.Font('Assets/Font/PressStart2P-Regular.ttf', 10)
x_resolution = 800; x = 8; chunk_x = 0
y_resolution = 600; y = 6; chunk_y = 0
...
controls_guide = ['Arrow Keys: Move', '', 'Spacebar: Interact', '', 'Return: View Inventory', '', 'A: View Atlas', '', '', '', 'Esc : Quit']
debug_mode = False

# Terrain Generation (adjustable to test game mechanics)

if debug_mode == True:
    nLand, nShore, nShadow, nCoin, nFish, nSeaweed, nCoral, nStore, nTree, nHole = 750, 900, 4000, 9000, 9350, 9100, 8000, 400, 1, 400
elif debug_mode == False:
    nLand, nShore, nShadow, nCoin, nFish, nSeaweed, nCoral, nStore, nTree, nHole = 750, 900, 902, 9970, 9350, 9100, 910, 897, 1, 748
...
def flip():
    ...
    total_h = line_h * len(controls_guide)
    start_y = display_offset[1] + (y_resolution - total_h) // 2
    for i, line in enumerate(controls_guide):
...
class Tile:
    def __init__(self, coors, is_seed, neighbors_seed):
        ...
        self.neighbors_seed = neighbors_seed
        self.randint = rr(1, 10000)
        self.navigable = None

        if self.is_seed == True:
            self.randint = 0
        if self.neighbors_seed == True:
            self.randint = rr(0,1000)

    class Surface:
        def __init__(self, coors, is_seed, neighbors_seed):
            Tile.__init__(self, coors, is_seed, neighbors_seed)
    ...
    class Subterrain:
        def __init__(self, coors, is_seed, neighbors_seed):
            Tile.__init__(self, coors, is_seed, neighbors_seed)
            self.worldtype = 'Subterrain'
            self.dark = (rr(10,50), rr(10,50), rr(10,50))
            self.bright = (rr(140,200), rr(140,200), rr(140,200))
            self.outline = (rr(10,11), rr(10,11), rr(10,11))
            self.goal = False

            if self.randint < nLand:
                self.tile_type = 'Bright'
                self.navigable = False
            else:
                self.tile_type = 'Dark'
                self.navigable = True
        ...
        def draw(self, a, b):
            if self.tile_type == 'Dark':
                DrawRect(self.dark, [a*scale, b*scale, 49, 49], 0)
                if self.goal == True:
                    DrawRect(self.outline, [a*scale, b*scale, 49, 49], 0)
                    DrawCirc((rr(200,201), rr(185,186), rr(0,1)), (a*scale + (scale/2-1), b*scale + (scale/2-1)), 20)
                    goal_text = small_font.render('Goal', True, (200, 100, 0))
                    goal_rect = goal_text.get_rect(); goal_rect.center = (a*scale + scale/2, (b*scale + scale/2) - 2)
                    Display.blit(goal_text, goal_rect)
            elif self.tile_type == 'Bright':
                DrawRect(self.bright, [a*scale, b*scale, 49, 49], 0)

class Chunk:
    def __init__(self, metcoords, worldtype):
        self.worldtype = worldtype
        self.grid = []
        self.seedmap = []
        self.x = metcoords[0]
        self.y = metcoords[1]
        tiles_by_worldtype = {'Surface': Tile.Surface, 'Subterrain': Tile.Subterrain}
        ...
        for x in range(0,16):
            ...
            for y in range(0,12):
                ...
                if island_seed < 40:
                    ...
                    island_seed = False
                column.append(island_seed)
            self.seedmap.append(column)
        for x in range(0,16):
            column = []
            for y in range(0,12):
                try:
                    neighbors = any(self.seedmap[x+dx][y+dy] for dx in (-1,0,1) for dy in (-1,0,1) if (dx,dy) != (0,0))
                except:
                    neighbors = False
                is_seed = self.seedmap[x][y]
                tile_coordinates_in_chunk = (x,y)
                column.append(tiles_by_worldtype[self.worldtype](tile_coordinates_in_chunk,is_seed,neighbors))
            self.grid.append(column)


class World:
    def __init__(self, worldtype):
        self.name = 'The largest scope of geometry in Treasure Hunt Simulator'
        self.worldtype = worldtype
        self.grid = self.build_worldgrid()

    def build_worldgrid(self):
        worldgrid = []
        for x in range(0,16):
            column = []
            for y in range(0,12):
                coor = (x,y)
                column.append(Chunk(coor, self.worldtype))
            worldgrid.append(column)
        return worldgrid

class Atlas:
    def __init__(self, world_to_draw):
        self.name = 'This is the Atlas, which draws a given world'
        self.atlas = world_to_draw
    
    def draw_atlas(self, coords, chunkcoords, blinking):
        atlas = self.atlas
        mapsea = []
        mapland = []
        mapshore = []
        ...
        for atlas_columns in atlas:
            for chunk in atlas_columns:
                chunk_x = chunk.x
                chunk_y = chunk.y
                for tile_columns in chunk.grid:
        ...
        if blinking == True:
            cx = x_resolution/8 + off + chunkcoords[0]*32 + coords[0]*2
            cy = y_resolution/4 + chunkcoords[1]*24 + coords[1]*2
...
def chunk_draw(current_chunk_grid):
    x = 0; y = 0
    for column in current_chunk_grid:
        y = 0
        for tile in column:
            tile.draw(x,y)
            y = y + 1
...
def item_draw(current_chunk_grid):
    x = 0; y = 0
    for column in current_chunk_grid:
        y = 0
        for tile in column:
            if tile.worldtype == 'Surface':
                if tile.coin == True:
                    DrawCoin(tile)
                if tile.store == True:
                    DrawStore(tile)
...
def move_handler(x, y, chunk_x, chunk_y, xbeforemove, ybeforemove, chunkxbeforemove, chunkybeforemove):
    if x != xbeforemove or y != ybeforemove:
        play_click()
    xbeforemove = x; ybeforemove = y
    chunkxbeforemove = chunk_x; chunkybeforemove = chunk_y
    return xbeforemove, ybeforemove, chunkxbeforemove, chunkybeforemove
...
def quest_handler(inventory, quest, selected_tile, landwalk):
    if selected_tile.worldtype == 'Surface':
        ...
        if inventory["Fishing Pole"] == 0:
            ...
            if selected_tile.fish == True:
                ...
                if landwalk == True:
                    ...
                    text = "Nothing's biting"
    elif selected_tile.worldtype == 'Subterrain':
        text = "It's dark down here"
...
def boundary_checker(active_worldgrid, landwalk, x, y, chunk_x, chunk_y, xbeforemove, ybeforemove, chunkxbeforemove, chunkybeforemove):
    if x == 16:
        chunk_x = chunk_x + 1
        if chunk_x == 16:
            chunk_x = 0
        x = 0
        if not landwalk and active_worldgrid[chunk_x][chunk_y].grid[x][y].navigable == False:
            chunk_x = chunkxbeforemove; x = xbeforemove
    if x == -1:
        chunk_x = chunk_x - 1
        if chunk_x == -1:
            chunk_x = 15
    ...
    if y == 12:
        ...
        y = 0
        if not landwalk and active_worldgrid[chunk_x][chunk_y].grid[x][y].navigable == False:
            chunk_y = chunkybeforemove; y = ybeforemove
    if y == -1:
        chunk_y = chunk_y - 1
        if chunk_y == -1:
            chunk_y = 11
        ...
        if not landwalk and active_worldgrid[chunk_x][chunk_y].grid[x][y].navigable == False:
            chunk_y = chunkybeforemove; y = ybeforemove
    return x, y, chunk_x, chunk_y

def display_landwalk_metrics(land_tally):
...
def land_tally_calculator(inp, land_tally):
    if inp == True:
